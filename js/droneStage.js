...
// Mobile intro length. Longer than before (was 2.5s, then 6.0s) because the
// route no longer ends inside the hero section — it now continues all the
// way down to the manifesto stage's screen position (see the "MOBILE
// TAKEOVER" note at the top of this file), so it needs a bit more time to
// still read as a smooth, unhurried glide rather than a rushed dash across
// two sections. Bumped 6.0 -> 7.5s together with the extended control2
// final-approach geometry below (see computeMobileHeroFlightConfig): the
// extra time isn't just "slower," it's time for the now-longer final
// stretch to actually play out at an unhurried pace instead of the drone
// sitting essentially parked while the ease finishes ticking t up to 1.
const HERO_FLIGHT_MOBILE_DURATION_SEC = 7.5;
...
function computeMobileHeroFlightConfig(stage) {
  ...
  // control2 shapes ONLY the flight's final approach (roughly its last
  // 25-30%, since it's the control point nearest FINAL on a cubic
  // Bezier) — control1 above is untouched, so the first ~70-75% of the
  // route still reads exactly as it did before. Previously control2 sat
  // very close to FINAL (-0.04/-0.08 of span), which starved that final
  // stretch of physical arc-length: because position is read via
  // curve.getPointAt(t) (arc-length parameterized), a short final
  // segment meant the drone had effectively finished its visible travel
  // well before t reached 1, even though the GSAP ease was still slowing
  // t down — reading as an early "TAK" park rather than a long final
  // glide. Pulling control2 further back along -span (more negative x)
  // and further above FINAL (more negative y) gives that last stretch
  // meaningfully more physical distance to cover, so the ease's natural
  // deceleration has room to play out as a real glide instead of an
  // already-arrived hover.
  const control2Screen = {
    x: points.endX + spanX * -0.16,
    y: points.endY + spanY * -0.24,
  };

  const control1Distance = THREE.MathUtils.lerp(startDistance, restDistance, 0.3);
  // Pulled back from 0.75 -> 0.6 in lockstep with control2Screen above:
  // keeps control2 a bit further from the resting camera distance too,
  // so the added final-approach length reads consistently in both
  // screen-space and depth instead of flattening out right at the end.
  const control2Distance = THREE.MathUtils.lerp(startDistance, restDistance, 0.6);
  const control1PlaneZ = THREE.MathUtils.lerp(MOBILE_START_PLANE_Z, 0, 0.3);
  const control2PlaneZ = THREE.MathUtils.lerp(MOBILE_START_PLANE_Z, 0, 0.6);
...
// rises smoothly across the first `fadeOutFrom` (~72%) of the flight,
// then eases back down to EXACTLY 0 over the final ~28% — at the same
...
const MOBILE_ORBIT_TUNING = {
  ...
  // Was 0.78. The final-approach segment (see computeMobileHeroFlightConfig's
  // control2) now spans roughly the flight's last 25-30% instead of a
  // shorter tail, so the fade start is pulled back to 0.72 to match: the
  // orbit still winds down smoothly over a comparable real-time window
  // (now ~28% of a 7.5s flight instead of ~22% of a 6.0s one), but that
  // window now lines up with the actual final-approach stretch instead of
  // only the last sliver of it — so the spin has fully settled to 0
  // *before* the heaviest part of the park deceleration, rather than still
  // finishing its own fade at the same moment position is settling.
  fadeOutFrom: 0.72, // progress at which the orbit starts easing back to 0
};
...
// MOBILE ONLY — its own separate handoff threshold (NOT shared with
// DESKTOP_IDLE_HANDOFF_T above — the two platforms' flights have
// different durations, curves, and rotation systems, so tying them to
// the same constant would be a coincidence, not a real relationship).
// updateHeroFlightYaw's lerp and updateHeroFlightOrbit's fade (see
// MOBILE_ORBIT_TUNING.fadeOutFrom) both asymptotically approach their
// resting values as t -> 1: by roughly t=0.95+ their combined angular
// velocity is already imperceptibly close to zero, well before the
// flight timeline's onComplete actually fires. Previously mobile let
// this play out all the way to t=1 and then jumped straight to
// startContinuousSpin() from onComplete — i.e. rotation.y genuinely sat
// at ~0 angular velocity for a real, visible stretch of time before the
// idle spin's constant-rate rotation kicked in, which is exactly the
// "geldi -> durdu -> döndü" pause this constant removes. Handing
// ownership to the idle spin at 0.88 instead — before that velocity
// decay happens — means rotation.y is always being actively driven by
// *something* (flight yaw+orbit, then idle spin) with no in-between
// frame where nothing is animating it. The trade-off (see updateHeroFlightOrbit's
// call site below) is a step change in *speed* right at the handoff —
// the cinematic orbit is turning noticeably faster than the idle spin's
// resting rate at 0.88 — but a speed change while still visibly
// rotating reads as far less jarring than motion actually stopping, and
// this is the same trade-off DESKTOP_IDLE_HANDOFF_T above already
// accepts. 0.88 is a starting point within the 0.85-0.92 range that
// still reads well on a real 390x844 render test; nudge it within that
// range if a specific device reads otherwise.
const MOBILE_IDLE_HANDOFF_T = 0.88;
...
function playHeroFlightIntro(stage, startAngleDeg, spinDurationSec, onComplete, idleHandoff = {}) {
  ...
  const tl = gsap.timeline({
    onComplete: () => {
      ...
      flightRig.rotation.set(0, 0, 0);
      setDistanceMultiplier(1);
      // Pin the resting yaw here ONLY if neither platform's early idle-
      // spin handoff actually fired (safety net for an extremely short/
      // throttled flight where onUpdate never got a frame past its
      // handoff threshold). Both MOBILE_IDLE_HANDOFF_T and
      // DESKTOP_IDLE_HANDOFF_T normally start idleHandoff.tween a little
      // before this fires — once that tween exists it already owns
      // spinRig.rotation.y and is actively mid-revolution, so forcing it
      // back to this fixed value here would itself reintroduce a visible
      // snap (this used to run unconditionally for mobile — `mobile ||
      // !idleHandoff.tween` — which is exactly what caused the reported
      // arrive -> pause -> spin seam: it stomped the early-handoff
      // tween's rotation right as it was already turning).
      if (!idleHandoff.tween) {
  ...
  tl.to(
    flightProgress,
    {
      ...
      // Mobile: power2.inOut (was sine.inOut). Both are smooth, built-in,
      // zero-velocity-at-the-ends eases — no bounce/elastic/back, nothing
      // custom — but power2's cubic falloff spends more relative time at
      // low speed near t=1 than sine's gentler curve does, which is
      // exactly what the longer control2 final-approach segment above
      // needs: a heavier, longer glide into the park position instead of
      // the flight reading as "done" a beat before t actually reaches 1.
      // This is also the same ease family desktop already uses below, so
      // it's a well-understood curve, not a new one-off variant.
      ease: 'power2.inOut',
      onUpdate: () => {
        ...
        if (mobile && t < MOBILE_IDLE_HANDOFF_T) {
          ...
          // line above. Stops being called entirely once t crosses
          // MOBILE_IDLE_HANDOFF_T below, so orbitState.applied's delta
          // bookkeeping is simply frozen at whatever it last reached —
          // it never gets to write spinRig.rotation.y again after the
          // idle spin takes ownership.
          updateHeroFlightOrbit(spinRig, t, orbitState, MOBILE_ORBIT_TUNING);
        } else if (mobile && !idleHandoff.tween) {
          // MOBILE ONLY — hand rotation.y ownership to the idle
          // continuous spin at MOBILE_IDLE_HANDOFF_T, continuing from
          // whatever angle the yaw settle + orbit have already reached
          // (preserveCurrentAngle: true — never reset to a fixed value).
          // From this frame on, neither updateHeroFlightYaw nor
          // updateHeroFlightOrbit is called again for the rest of this
          // timeline (see the branch above) — ownership of
          // spinRig.rotation.y is one-way: flight rotation system ->
          // idle continuous spin, never back, never both at once.
          idleHandoff.tween = startContinuousSpin(spinRig, startAngleDeg, spinDurationSec, {
            preserveCurrentAngle: true,
          });
        } else if (!mobile && t < DESKTOP_IDLE_HANDOFF_T) {
          updateHeroFlightYaw(spinRig, t, targetYawRad, cfg.yawSettle);
        } else if (!mobile && !idleHandoff.tween) {
 ...
 *
 * EXCEPTION: the mobile hero flight stage (flightIntro: true, mobile
 * viewport) never pauses this way. Its `container` stays up in the hero
 * section while the drone itself is visually carried down into the
 * manifesto park spot by the mobile takeover overlay, so `container`'s
 * own on/off-screen state has nothing to do with whether the drone is
 * actually visible. See the `keepMobileHeroAlive` comment below.
 */
async function createStage(
  ...
  // MOBILE HERO VISIBILITY EXCEPTION
  // --------------------------------
  // On mobile the hero flight's final "parked" pose is NOT inside
  // `container` (the original .hero-drone-stage element) any more — the
  // mobile takeover overlay (see getMobileHeroScreenPoints / the "MOBILE
  // TAKEOVER" note at the top of this file) visually carries the drone
  // down into the manifesto section's screen position, while `container`
  // itself stays put in normal document flow, up near the top of the
  // page. So once the user scrolls the hero section itself out of view,
  // an IntersectionObserver watching `container` reports
  // entry.isIntersecting === false even though the drone is still
  // clearly on screen (parked over the manifesto copy) — which used to
  // pause both the continuous spin tween and stage.render(), making the
  // drone freeze solid until a small upward scroll brought `container`
  // back into view and snapped the spin back on.
  //
  // This is only ever true for the one flightIntro stage, and only on
  // mobile: on desktop the hero drone never leaves its own container, so
  // desktop's IntersectionObserver-driven pause/resume is left completely
  // untouched below. Computed once at stage-creation time — same as the
  // existing mobile/desktop split in initDroneStages() (e.g. the
  // captureContainer mobile-skip), which is also a one-time snapshot, not
  // rebuilt on resize.
  const keepMobileHeroAlive = flightIntro && isMobileViewport();
  ...
  function applyVisibility() {
    ...
    // Mobile hero stage: never let IntersectionObserver pause the spin —
    // see the keepMobileHeroAlive comment above.
    if (keepMobileHeroAlive) {
  ...
  if (flightIntro) {
    ...
    startIntro = () => {
      // playHeroFlightIntro may already have started the idle continuous
      // spin itself a little before its timeline finishes — desktop via
      // DESKTOP_IDLE_HANDOFF_T, mobile via its own MOBILE_IDLE_HANDOFF_T
      // — writing its tween into this shared object as soon as it does.
      // That's what closes the stop-then-spin gap on both platforms. The
      // `idleHandoff.tween ||` fallback below only ever matters as a
      // safety net (e.g. an extremely short/throttled flight where
      // onUpdate never got a frame past its handoff threshold) — in the
      // normal case idleHandoff.tween is already set by the time this
      // callback runs, so this just adopts that same tween rather than
  ...
  // Mobile hero stage deliberately gets no observer at all: `container`
  // (the original .hero-drone-stage element) is the wrong thing to watch
  // once the drone has visually handed off to the manifesto park spot, so
  // we don't even wire up isIntersecting for it. Desktop, and every other
  // stage (manifesto, cinematic), are completely unaffected — they keep
  // this exact IntersectionObserver-driven pause/resume as before.
  if ('IntersectionObserver' in window && !keepMobileHeroAlive) {
  ...
  function render() {
    // Mobile hero stage: keep rendering regardless of `isVisible` — the
    // drone is visually parked over the manifesto section even while
    // `container` itself (up in the hero section) is scrolled off-screen.
    if (keepMobileHeroAlive || isVisible) stage.render();
  }
